...
def validate_video_derived_row(job: dict[str, object], *, source: Path, output: Path, job_id: str) -> None:
    if job.get("kind") != "row-strip":
        raise SystemExit(f"job {job_id} may not use video-derived row provenance")
    expected_output_hash = job.get("output_sha256")
    if not isinstance(expected_output_hash, str) or not expected_output_hash:
        raise SystemExit(f"job {job_id} video-derived row is missing output_sha256")
    if not output.is_file():
        raise SystemExit(f"job {job_id} video-derived decoded output is missing: {output}")
    if file_sha256(output) != expected_output_hash:
        raise SystemExit(
            f"job {job_id} video-derived decoded output hash does not match imagegen-jobs.json; "
            "rerun process_power_frames.py"
        )
    if not source.is_file():
        raise SystemExit(f"job {job_id} source video no longer exists: {source}")
    report_raw = job.get("video_processing_report")
    if isinstance(report_raw, str) and report_raw:
        report = Path(report_raw).expanduser()
        if not report.is_absolute():
            report = output.parents[1] / report
        if not report.resolve().is_file():
            raise SystemExit(f"job {job_id} video processing report is missing: {report}")
...
def validate_completed_job_source(
    ...
    *,
    ...
    if job.get("source_provenance") == "video-derived-row":
        validate_video_derived_row(job, source=source, output=output, job_id=job_id)
        return
